...
//		Statement st = null;
		...
		try(Connection con = DriverManager.getConnection("jdbc:mysql://localhost/mydb1","root","root");
	...
	private static void InsertintoDB() {
		try(Connection con = DriverManager.getConnection("jdbc:mysql://localhost/mydb1","root","root");
				Statement st = con.createStatement();) {
			int rowsinserted = st.executeUpdate("insert into account values(2,'sita','ar',10000)");
